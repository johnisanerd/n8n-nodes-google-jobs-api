...
import { ACTOR_ID } from '../ApifyGoogleJobs.node';
import { buildActorInput } from '../ApifyGoogleJobs.properties';
...
/**
 * Shape a single job dataset item according to the chosen Output mode.
...
function shapeItem(
	...
	// simplified
	const detected = (item.detected_extensions ?? {}) as Record<string, any>;
	const applyOptions = (item.apply_options ?? []) as Array<Record<string, any>>;
	const applyLink = applyOptions[0]?.link ?? item.source_link;
	return {
		title: item.title,
